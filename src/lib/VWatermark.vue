<template>
  <div style="position:relative;" ref="wrapper">
    <slot></slot>
    <div v-for="(_, row_key) in new Array(rowInt)"  :key="row_key">
      <div v-for="(_, column_key) in new Array(columnInt)"  :key="column_key" :style="{
    ...
    }" style="position:absolute;z-index:999;display:inline-block;overflow:hidden;text-align:center;pointer-events:none;">
        {{text}}
      </div>
    </div>
  </div>
